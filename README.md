...
Projekt jest dostępny na licencji XXX (will be chosen later). Szczegóły w pliku [LICENSE](./LICENSE).
...
The project is available under the XXX license (will be chosen later). Details in the [LICENSE](./LICENSE) file.
...
Проект доступен по лицензии XXX (will be chosen later). Подробности в файле [LICENSE](./LICENSE).